...
class TestSpec:
    ...
    def get_entity_sorter_func(self, json_ensure_ascii=False):
        """ "By default we sort by the '_id' field, but in some cases that field is autogenerated
        in non-deterministic ways. (For instance in Sesam Talk *-share pipes)."""
        fields_to_sort_by = self._spec.get("fields_to_sort_by", ["_id"])

        def entity_sorter_func(_e):
            sort_key = []
            for field in fields_to_sort_by:
                fieldvalue = _e.get(field)
            ...
            sort_key.append(json.dumps(_e, ensure_ascii=json_ensure_ascii, sort_keys=True))
            return sort_key
