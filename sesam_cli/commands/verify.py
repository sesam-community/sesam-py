...
def execute_verify(client, base_dir, git_root):
    ...
    with profile_phase(client, "verify.collect_output_pipes"):
        for pipe in client.sesam_node.get_output_pipes() + client.sesam_node.get_endpoint_pipes():
            if pipe.runtime.get("is-valid-config", False) is False:
                client.logger.error(
                    "The pipe '%s' has invalid config, cannot verify pipe!" % pipe.id
                )
                client.logger.error(
    ...
    with profile_phase(client, "verify.load_test_specs"):
    ...
    failed_tests = []
    missing_tests = []
    with profile_phase(client, "verify.compare_outputs"):
    ...
    if failed:
        if len(failed_tests) > 0:
            ...
            for failed_test_spec in failed_tests:
                client.logger.error("%s (%s)" % (failed_test_spec.pipe, failed_test_spec.spec_file))
